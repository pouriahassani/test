...
if test == 1:
    ...
    CHARGE_STEPS = 20 #Increase the charging rate in CHARGE_STEPS and see the paritiong sucess rate change
    CHARGING_RATE = 0.1
    CHARGE_INITIAL = 2000

if test == 0:
    STEPS = 400  # Number of steps to shift the window of values
    # The application's execution time (Ci) will be multiplied by a random value picked from the window of values
    # to generate corresponding deadlines and periods.
    ...
    # staring window will be [1:6]. In the step number s=n 
    # the window will be [n*SHIFT_WINDOW+START_OF_WINDOW,n*SHIFT_WINDOW+START_OF_WINDOW + WINDOW_BOUND ]
    ITR_PER_WINDOW = 500 #Number of iterarions per window to reach reliable results
    CHARGE_STEPS = 20 #Increase the charging rate in CHARGE_STEPS and see the paritiong sucess rate change
    CHARGING_RATE = 0.1
...
for charge_step in range(CHARGE_STEPS):
    ...
    for step in range(STEPS):
        ...
        if EXE_FULL_PARTITIONING >=3 and AXE_FULL_PARTITIONING >= 3:
            ...
            print(args.Start_Charge,SYSTEM_AXE.CHARGING_RATE,start_window,EXE_partition_result,AXE_partition_result)


# Writing to CSV
if test:
    AXE_FILENAME = "OUTPUT/AXE_test_" + str(args.Start_Charge) + ".csv"
    EXE_FILENAME = "OUTPUT/EXE_test_" + str(args.Start_Charge) + ".csv"
else:
    AXE_FILENAME = "OUTPUT/AXE_" + str(args.Start_Charge) + ".csv"
    EXE_FILENAME = "OUTPUT/EXE_" + str(args.Start_Charge) + ".csv"
