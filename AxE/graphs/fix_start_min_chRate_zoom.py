...
for i, start_val_target in enumerate(start_values):
    ...
    if  not FULL_PARTITIONING:
      ...
      if ZOOM:
         ...
         ax.plot(zoom_charge_exe,  zoom_result_exe, label="E-MPSoC",  color = 'r',marker='o', linestyle='-',markersize=3)
         ax.plot(zoom_charge_faxe, zoom_result_faxe, label=" AxE-F",color = 'g',marker='s', linestyle='--',markersize=3)
      if not ZOOM:
         ax.plot(filtered_charge_exe, filtered_result_exe,   label="E-MPSoC", color = 'r', marker='o', linestyle='-',markersize=3)
         ax.plot(filtered_charge_faxe, filtered_result_faxe, label=" AxE-F",color = 'g', marker='s', linestyle='--',markersize=3)
      ax.set_xlabel("Battery Charging Rate (P) in mW",fontsize=20)
      ax.set_ylabel("Partitioning Success Rate (%)", fontsize=20)
      ax.set_title(f"start_window = {start_val_target:.1f}", fontsize=22)
