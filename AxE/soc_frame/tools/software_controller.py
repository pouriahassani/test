...
class Controller( Software ):
    ...
    def create_prg_define( self, arch_lst, mem, offset = 0, deadline_is_wcet_times_this_number = 1 ):
        ...
        # Instead of using controller-relative addresses, place programs in proper node memory spaces
        # Each program gets placed at: (program_index + 1) * AXI_OFFSET_STEP
        
        prgs_lst = mem.get_prgs_lst()
        
        define_lst.append( '/* This file has been automatically generated */' )
        define_lst.append( '' )
        define_lst.append( '#include "../../../systems/2x2_16_priority_input_approx/generated/axe_memory_layout.h"' )
        ...
        for i, prg in enumerate( prgs_lst ):
            ...
            for arch in arch_lst:
                ...
                # Place program at proper node memory space: (program_index + 1) * AXI_OFFSET_STEP
                # This is both where the program is stored AND the AXI offset for the node
                node_memory_base = (i + 1) * 0x01000000  # AXI_OFFSET_STEP = 0x01000000
                
                define_lst.append( '\tprgs[' + str(i) + '].addr[ARCH_' + arch.upper() + '] = ' + hex(node_memory_base) + '; // Node ' + str(i) + ' memory space' )
