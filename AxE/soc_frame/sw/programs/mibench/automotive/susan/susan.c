...
// Debug macro for SUSAN execution tracking
#define DEBUG_SUSAN
#ifdef DEBUG_SUSAN
...
usage()
{
  DEBUG_PRINT("usage function called");
...
int getint(fd)
  FILE *fd;
{
  DEBUG_PRINT("getint function started");
  int c, i;
  char dummy[10000];
...
void get_image(filename,in,x_size,y_size)
  ...
  int            *x_size, *y_size;
{
DEBUG_PRINT("get_image function started");
FILE  *fd;
char header [100];
int  tmp;
...
put_image(filename,in,x_size,y_size)
  ...
  int  x_size,
       y_size;
{
DEBUG_PRINT("put_image function started");
FILE  *fd;
...
int_to_uchar(r,in,size)
  ...
  int   *r, size;
{
DEBUG_PRINT("int_to_uchar conversion started");
int i,
    max_r=r[0],
    min_r=r[0];
...
void setup_brightness_lut(bp,thresh,form)
  ...
  int   thresh, form;
{
DEBUG_PRINT("setup_brightness_lut function started");
int   k;
float temp;
...
{
DEBUG_PRINT("susan_principle function started");
int   i, j, n;
uchar *p,*cp;
...
susan_principle_small(in,r,bp,max_no,x_size,y_size)
  ...
  int   *r, max_no, x_size, y_size;
{
DEBUG_PRINT("susan_principle_small function started");
int   i, j, n;
uchar *p,*cp;
...
uchar median(in,i,j,x_size)
  ...
  int   i, j, x_size;
{
DEBUG_PRINT("median function started");
int p[8],k,l,tmp;
...
DEBUG_PRINT("susan_smoothing function started");
...
/* }}} */
  ...
  DEBUG_PRINT("About to allocate tmp_image");
  printf("[DEBUG] Calculating tmp_image size: (%d + %d*2) * (%d + %d*2) = %d bytes\n", 
         x_size, mask_size, y_size, mask_size, 
         (x_size+mask_size*2) * (y_size+mask_size*2));
  tmp_image = (uchar *) malloc( (x_size+mask_size*2) * (y_size+mask_size*2) );
  DEBUG_PRINT("tmp_image allocation completed");
...
/* }}} */
  ...
  DEBUG_PRINT("About to allocate distance LUT");
  printf("[DEBUG] Allocating distance LUT: %d * %d = %d bytes\n", 
         n_max, n_max, n_max*n_max);
  dp     = (unsigned char *)malloc(n_max*n_max);
  DEBUG_PRINT("Distance LUT allocation completed");
  dpt    = dp;
  temp   = -(dt*dt);
...
{
DEBUG_PRINT("edge_draw function started");
int   i;
uchar *inp, *midp;
...
susan_thin(r,mid,x_size,y_size)
  ...
  int   *r, x_size, y_size;
{
DEBUG_PRINT("susan_thin function started");
...
susan_edges(in,r,mid,bp,max_no,x_size,y_size)
  ...
  int   *r, max_no, x_size, y_size;
{
DEBUG_PRINT("susan_edges function started");
float z;
...
{
DEBUG_PRINT("corner_draw function started");
uchar *p;
int   n=0;
...
susan_corners(in,r,bp,max_no,corner_list,x_size,y_size)
  ...
  CORNER_LIST corner_list;
{
DEBUG_PRINT("susan_corners function started");
int   n,x,y,sq,xx,yy,
      i,j,*cgx,*cgy;
float divide;
uchar c,*p,*cp;
...
DEBUG_PRINT("SUSAN main function started");
...
/* }}} */
  ...
  DEBUG_PRINT("Image loaded successfully");
...
/* }}} */
  ...
  switch (mode)
  {
    case 0:
      /* {{{ smoothing */
      DEBUG_PRINT("Starting smoothing mode");
      DEBUG_PRINT("Setting up brightness LUT for smoothing");
      setup_brightness_lut(&bp,bt,2);
      DEBUG_PRINT("Starting SUSAN smoothing algorithm");
      susan_smoothing(three_by_three,in,dt,x_size,y_size,bp);
      DEBUG_PRINT("Smoothing completed successfully");
      break;

/* }}} */
    case 1:
      /* {{{ edges */
      DEBUG_PRINT("Starting edge detection mode");
      DEBUG_PRINT("Allocating memory for edge response");
      r   = (int *) malloc(x_size * y_size * sizeof(int));
      DEBUG_PRINT("Setting up brightness LUT for edge detection");
      setup_brightness_lut(&bp,bt,6);

      if (principle)
      {
        DEBUG_PRINT("Running principle component mode");
        if (three_by_three) {
          DEBUG_PRINT("Using small 3x3 principle");
          susan_principle_small(in,r,bp,max_no_edges,x_size,y_size);
        } else {
          DEBUG_PRINT("Using full principle");
          susan_principle(in,r,bp,max_no_edges,x_size,y_size);
        }
        DEBUG_PRINT("Converting int response to uchar");
        int_to_uchar(r,in,x_size*y_size);
      }
      else
      {
        DEBUG_PRINT("Running full edge detection mode");
        mid = (uchar *)malloc(x_size*y_size);
        memset (mid,100,x_size * y_size); /* note not set to zero */

        if (three_by_three) {
          DEBUG_PRINT("Using small 3x3 edge detection");
          susan_edges_small(in,r,mid,bp,max_no_edges,x_size,y_size);
        } else {
          DEBUG_PRINT("Using full edge detection");
          susan_edges(in,r,mid,bp,max_no_edges,x_size,y_size);
        }
        if(thin_post_proc) {
          DEBUG_PRINT("Applying thinning post-processing");
          susan_thin(r,mid,x_size,y_size);
        }
        DEBUG_PRINT("Drawing edge results");
        edge_draw(in,mid,x_size,y_size,drawing_mode);
      }
      DEBUG_PRINT("Edge detection completed successfully");
      break;

/* }}} */
    case 2:
      /* {{{ corners */
      DEBUG_PRINT("Starting corner detection mode");
      DEBUG_PRINT("Allocating memory for corner response");
      r   = (int *) malloc(x_size * y_size * sizeof(int));
      DEBUG_PRINT("Setting up brightness LUT for corner detection");
      setup_brightness_lut(&bp,bt,6);

      if (principle)
      {
        DEBUG_PRINT("Running principle component mode for corners");
        susan_principle(in,r,bp,max_no_corners,x_size,y_size);
        DEBUG_PRINT("Converting corner response to uchar");
        int_to_uchar(r,in,x_size*y_size);
      }
      else
      {
        if(susan_quick) {
          DEBUG_PRINT("Running quick corner detection");
          susan_corners_quick(in,r,bp,max_no_corners,corner_list,x_size,y_size);
        } else {
          DEBUG_PRINT("Running full corner detection");
          susan_corners(in,r,bp,max_no_corners,corner_list,x_size,y_size);
        }
        DEBUG_PRINT("Drawing corner results");
        corner_draw(in,corner_list,x_size,drawing_mode);
      }
      DEBUG_PRINT("Corner detection completed successfully");
      break;
...
/* }}} */
  ...
  DEBUG_PRINT("Writing output image");
  put_image(argv[2],in,x_size,y_size);
  DEBUG_PRINT("SUSAN main function completed");
}
