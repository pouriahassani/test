...
{
    // diff: e 46137 - a 43137
    asm __volatile__ ( "mul x10, x11, x12" );
    asm __volatile__ ( "addi %0, x10, 0" : "=r" (rd) );
    
    // diff: e 54337 - a 43137
    //~ asm __volatile__ ( "mul %0, %1, %2" : "=r" (rd) : "r" (rs1), "r" (rs2) );
    //~ asm __volatile__ ( "addi %0, %1, 0" : "=r" (rd) : "r" (rd) );
    
    // diff: e 48937 - a 43137
    //~ rd = rs1 * rs2;
    //~ rd = rd + 0;
...
void img_test2()
{
        ...
        for ( h = 0; h < HEIGHT; h++ )
        {
            ...
            for ( fw = 0; fw < 3; fw++ )
            {
                for ( fh = 0; fh < 3; fh++ )
                {
                    ...
                    //~ print_signed( cr );
                    //~ print_str( " * " );
                    //~ print_signed( filter[fw][fh] );
                    //~ print_str( " = " );
                    //~ print_signed( pro_appr ); NL;
    ...
    print_str("cnt");print_dec( cnt_global.cnt );NL;
    print_str("ovr");print_dec( cnt_global.overflows );NL;
...
//apprx cnt: 1.003.380.076
//exact cnt: 1.016.727.009

//diff: 13.346.933 clk cnt
//diff: 533.87732 ms with 25 MHz
...
void my_main()
{
    ...
    while ( 1 == 1 )
    {
        ...
        cnt_end = GET_COUNTER_GLOBAL;
        cnt_diff2 = cnt_end - cnt_start;
        
        //~ time_update_global();
        //~ time_record_global_cnt( &cnt_end ); 
        
        //~ cnt_diff = time_diff( &cnt_end, &cnt_start );
        print_str( " diff: e " );print_dec( cnt_diff1 );
        print_str( " - a " );print_dec( cnt_diff2 ); NL;
        
        print_str( " diff: e " );print_dec( pro );
        print_str( " - a " );print_dec( pro_appr ); NL;
    ...
    //~ // print_str( "dec -1: " ); print_dec( rs1 );NL;
    //~ // print_str( "sig -1: " ); print_signed( rs1 );NL;
